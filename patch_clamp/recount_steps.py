"""re-count patch spike data. suspect problems"""
import patch_clamp.steps as steps
import patch_clamp.database as db
import patch_clamp.archive as archive

SCHEMA = archive.get_schema("patch_clamp/peaks_recount.sqlite")
con, cur = archive.make_db(db.DATABASE_PATH, SCHEMA)
cur.close()
con.close()


QUERY_STRING = "INSERT INTO peaks_recount (fname, fpath, sweep, peak_index) VALUES (:name, :path, :sweep, :peaks) ON CONFLICT DO NOTHING"
